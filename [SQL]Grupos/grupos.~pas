 ...
 ######################################################################}
  ...
  ixCampos  : array[1..ixMaximo] of TCampoDes =
     ( (rotulo: 'Código'       ; fisico: 'CODIGO'; iTab: 1),
       (rotulo: 'Nome do Grupo'; fisico: 'GRUPO' ; iTab: 1) );
...
procedure TformGrupos.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformGrupos.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformGrupos.btnSairClick(Sender: TObject);
begin
//     close;
end;

procedure TformGrupos.FormCreate(Sender: TObject);
begin
...
procedure TformGrupos.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformGrupos.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformGrupos.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformGrupos.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformGrupos.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformGrupos.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformGrupos.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformGrupos.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformGrupos.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformGrupos.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
    Dataset.fieldByName(ixCampos[1].fisico).AsInteger :=
      dmoPrincipal.LerSequencia(ixTabelas[1]);
end;

procedure TformGrupos.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformGrupos.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformGrupos.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformGrupos.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformGrupos.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
end;

procedure TformGrupos.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformGrupos.btnImprimirClick(Sender: TObject);
begin
...
procedure TformGrupos.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformGrupos.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
