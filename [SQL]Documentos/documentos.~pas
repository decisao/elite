...
procedure TformDocumentos.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformDocumentos.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformDocumentos.FormShow(Sender: TObject);
var
  i: byte;
  myEnter: char;
begin
...
procedure TformDocumentos.btnSairClick(Sender: TObject);
begin
//     close;
end;
...
procedure TformDocumentos.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformDocumentos.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformDocumentos.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformDocumentos.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformDocumentos.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformDocumentos.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformDocumentos.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformDocumentos.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformDocumentos.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformDocumentos.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
    with Dataset do
     begin
       ...
       fieldByName('INFORMACOES').AsString := 'nada';

     end;

end;

procedure TformDocumentos.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformDocumentos.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformDocumentos.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformDocumentos.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformDocumentos.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformDocumentos.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformDocumentos.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformDocumentos.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformDocumentos.btnSalvarClick(Sender: TObject);
const
     myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformDocumentos.FormDestroy(Sender: TObject);
begin
...
procedure TformDocumentos.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformDocumentos.btnImprimirClick(Sender: TObject);
begin
...
procedure TformDocumentos.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformDocumentos.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
