...
type
  TformContasRecebidas = class(TForm)
    ...
    procedure ReimprimiroRecibo1Click(Sender: TObject);
  private
...
uses data_principal, documentos, os, centros, bancos, clientes,
  plancontas, imprimirrecibo;
...
procedure TformContasRecebidas.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformContasRecebidas.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformContasRecebidas.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformContasRecebidas.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformContasRecebidas.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformContasRecebidas.cmbSelecionarChange(Sender: TObject);
begin
     edtSelecionar.Clear;
     edtSelecionar.SetFocus;
end;

procedure TformContasRecebidas.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformContasRecebidas.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformContasRecebidas.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformContasRecebidas.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformContasRecebidas.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformContasRecebidas.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformContasRecebidas.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
   qrySelecionar.tag := 0;
...
procedure TformContasRecebidas.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
    with Dataset do
     begin
...
procedure TformContasRecebidas.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformContasRecebidas.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformContasRecebidas.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformContasRecebidas.qryDetalhesAfterOpen(DataSet: TDataSet);
begin
   with Dataset do
    begin
...
procedure TformContasRecebidas.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformContasRecebidas.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformContasRecebidas.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformContasRecebidas.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformContasRecebidas.btnSalvarClick(Sender: TObject);
const
     myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformContasRecebidas.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformContasRecebidas.cmbDataChange(Sender: TObject);
var
  Key: char;
begin
  Key := #13;
...
procedure TformContasRecebidas.DateEdit1KeyPress(Sender: TObject;
  var Key: Char);
begin
  edtSelecionarKeyPress(Sender, Key);
end;

procedure TformContasRecebidas.Label11Click(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
...
procedure TformContasRecebidas.lblVendaOSClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
...
procedure TformContasRecebidas.fldCODDOCUMENTOmudar(Sender: TField);
begin
...
procedure TformContasRecebidas.fldCODCLIENTEmudar(Sender: TField);
begin
  if Sender.AsString > '' then
   with qryCliente do
    begin
...
procedure TformContasRecebidas.fldCH_BANCOmudar(Sender: TField);
begin
  if Sender.AsString > '' then
   with qryBanco do
    begin
...
procedure TformContasRecebidas.fldCODPLANOmudar(Sender: TField);
begin
  if Sender.AsString > '' then
   with qryPlano do
    begin
...
procedure TformContasRecebidas.ckEmpresaClick(Sender: TObject);
const
  key: char = #13;
begin
  edtSelecionarKeyPress(Sender, Key);
...
procedure TformContasRecebidas.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformContasRecebidas.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [];
end;

procedure TformContasRecebidas.FormDestroy(Sender: TObject);
begin
...
procedure TformContasRecebidas.qryMovimentoAfterOpen(DataSet: TDataSet);
begin
  with Dataset do
...
procedure TformContasRecebidas.qryDocumentoAfterOpen(DataSet: TDataSet);
begin
  (Dataset as TIBQuery).FetchAll;
end;

procedure TformContasRecebidas.qryDetalhesBeforeOpen(DataSet: TDataSet);
begin
...
procedure TformContasRecebidas.qryDetalhesAfterClose(DataSet: TDataSet);
begin
...
procedure TformContasRecebidas.Label17Click(Sender: TObject);
begin
...
procedure TformContasRecebidas.Label8Click(Sender: TObject);
begin
...
procedure TformContasRecebidas.lblPessoaClick(Sender: TObject);
begin
...
procedure TformContasRecebidas.Label20Click(Sender: TObject);
begin
...
procedure TformContasRecebidas.btnExcluirClick(Sender: TObject);
begin
...
procedure TformContasRecebidas.Grade1Click(Sender: TObject);
begin
...
procedure TformContasRecebidas.ReimprimiroRecibo1Click(Sender: TObject);
begin
   with TformImprimirRecibo.Create(Self) do
    try
      inCODMOVIMENTO := Self.qrySelecionar.fieldByName('CODMOVIMENTO').AsInteger;
      inNUMERONOTA   := Self.qrySelecionar.fieldByName('CODIGO').AsInteger;
      showmodal;
    finally
      release;
    end;
