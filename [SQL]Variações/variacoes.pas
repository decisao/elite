...
{######################################################################
 ...
 ######################################################################}
  ...
  ixCampos  : array[1..ixMaximo] of TCampoDes =
     ( (rotulo: 'Data/hora do reajuste'; fisico: 'DATA'         ; iTab: 1),
       (rotulo: 'Valor anterior'       ; fisico: 'VALOR_ANTIGO' ; iTab: 1),
       (rotulo: 'Valor reajustado'     ; fisico: 'VALOR_NOVO'   ; iTab: 1),
       (rotulo: 'Reajuste'             ; fisico: 'REAJUSTE'     ; iTab: 1) );
...
procedure TformReajustes.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformReajustes.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformReajustes.FormCreate(Sender: TObject);
begin
...
procedure TformReajustes.edtSelecionarKeyPress(Sender: TObject);
var
  i, sel: byte;
  StrSQL: string;
begin
...
procedure TformReajustes.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformReajustes.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformReajustes.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformReajustes.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformReajustes.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformReajustes.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformReajustes.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformReajustes.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
     ...
     with Dataset do
      begin
       ...
       { mascaras }
       (fieldByName('REAJUSTE') as TNumericField).DisplayFormat := '##0.00 ''%''';
       (fieldByName('REAJUSTE') as TNumericField).EditFormat := '##0.00';
       (fieldByName('VALOR_ANTIGO') as TIBBCDField).Currency := True;
       (fieldByName('VALOR_NOVO') as TIBBCDField).Currency := True;

      end;
end;

procedure TformReajustes.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
    with Dataset do
...
procedure TformReajustes.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformReajustes.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformReajustes.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformReajustes.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
     ...
     with qryDetalhes do
      begin
       ...
       ParamByName(ixCampos[1].fisico).AsDateTime :=
         qrySelecionar.fieldByName(ixCampos[1].fisico).AsDateTime;
       Open;
      end;
...
procedure TformReajustes.qryDetalhesAfterOpen(DataSet: TDataSet);
begin
     with Dataset do
      begin
...
procedure TformReajustes.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformReajustes.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformReajustes.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformReajustes.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformReajustes.FormDestroy(Sender: TObject);
begin
...
procedure TformReajustes.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformReajustes.btnImprimirClick(Sender: TObject);
begin
...
procedure TformReajustes.btnModificarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformReajustes.btnModificarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
