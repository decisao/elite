...
{######################################################################
 ...
 ######################################################################}
  ...
  ixCampos  : array[1..ixMaximo] of TCampoDes =
     ( (rotulo: 'Data/hora do movimento'; fisico: 'DATA'            ; iTab: 1),
       (rotulo: 'Estoque anterior'      ; fisico: 'ESTOQUEANTERIOR' ; iTab: 1),
       (rotulo: 'Estoque reajustado'    ; fisico: 'ESTOQUEATUAL'    ; iTab: 1),
       (rotulo: 'Quantidade'            ; fisico: 'MOVIMENTO'       ; iTab: 1) );
...
procedure TformHistoque.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformHistoque.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformHistoque.FormCreate(Sender: TObject);
begin
...
procedure TformHistoque.edtSelecionarKeyPress(Sender: TObject);
var
  i, sel: byte;
  StrSQL: string;
begin
...
procedure TformHistoque.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformHistoque.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformHistoque.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformHistoque.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformHistoque.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformHistoque.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformHistoque.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformHistoque.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
     ...
     with Dataset do
      begin
       ...
       { mascaras }
       (fieldByName('MOVIMENTO') as TNumericField).DisplayFormat := '##0.00';
       (fieldByName('MOVIMENTO') as TNumericField).EditFormat := '##0.00';
       (fieldByName('ESTOQUEANTERIOR') as TNumericField).DisplayFormat := '##0.00';
       (fieldByName('ESTOQUEANTERIOR') as TNumericField).EditFormat := '##0.00';
       (fieldByName('ESTOQUEATUAL') as TNumericField).DisplayFormat := '##0.00';
       (fieldByName('ESTOQUEATUAL') as TNumericField).EditFormat := '##0.00';

      end;
end;

procedure TformHistoque.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
    with Dataset do
...
procedure TformHistoque.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformHistoque.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformHistoque.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformHistoque.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
     ...
     with qryDetalhes do
      begin
       ...
       ParamByName(ixCampos[3].fisico).AsFloat :=
         qrySelecionar.fieldByName(ixCampos[3].fisico).AsFloat;
       Open;
      end;
...
procedure TformHistoque.qryDetalhesAfterOpen(DataSet: TDataSet);
begin
     with Dataset do
      begin
...
procedure TformHistoque.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformHistoque.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformHistoque.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformHistoque.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformHistoque.FormDestroy(Sender: TObject);
begin
...
procedure TformHistoque.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformHistoque.btnImprimirClick(Sender: TObject);
begin
...
procedure TformHistoque.btnModificarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformHistoque.btnModificarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
