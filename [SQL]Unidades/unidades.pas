...
{######################################################################
 ...
 ######################################################################}
  ...
  ixCampos  : array[1..ixMaximo] of TCampoDes =
     ( (rotulo: 'Sigla'            ; fisico: 'SIGLA'  ; iTab: 1),
       (rotulo: 'Unidade de Medida'; fisico: 'UNIDADE'; iTab: 1) );
...
procedure TformUnidades.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformUnidades.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformUnidades.btnSairClick(Sender: TObject);
begin
//     close;
end;
...
procedure TformUnidades.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformUnidades.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformUnidades.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformUnidades.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformUnidades.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformUnidades.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformUnidades.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformUnidades.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformUnidades.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformUnidades.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformUnidades.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformUnidades.qryDetalhesAfterOpen(DataSet: TDataSet);
begin
     { mascaras }
     with Dataset do
      begin
...
procedure TformUnidades.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformUnidades.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformUnidades.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformUnidades.btnImprimirClick(Sender: TObject);
begin
...
procedure TformUnidades.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformUnidades.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
