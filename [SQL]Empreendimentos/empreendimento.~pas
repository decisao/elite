...
uses
  ...
  TB97Ctls, ppCtrls, ppPrnabl, ppClass, ppModule, daDatMod, ppBands,
  ppProd, ppReport, ppComm, ppRelatv, ppCache, ppDB, ppDBPipe, ppDBBDE, IBUpdateSQL,
  IBDatabase, gridreport;

type
  TformEmpreendimentos = class(TForm)
    ...
    traRelatorio: TIBTransaction;
    Grade1: TMenuItem;
    N2: TMenuItem;
    RelatriodeClientesreas1: TMenuItem;
    Dock974: TDock97;
    ...
    procedure RelatriodeClientesreas1Click(Sender: TObject);
...
var
  formEmpreendimentos: TformEmpreendimentos;

implementation

uses data_principal, areasemp;
...
procedure TformEmpreendimentos.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformEmpreendimentos.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformEmpreendimentos.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformEmpreendimentos.btnSairClick(Sender: TObject);
begin
//     close;
end;

procedure TformEmpreendimentos.FormCreate(Sender: TObject);
begin
...
procedure TformEmpreendimentos.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformEmpreendimentos.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformEmpreendimentos.cmbSelecionarChange(Sender: TObject);
begin
     edtSelecionar.Clear;
     edtSelecionar.SetFocus;
end;

procedure TformEmpreendimentos.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformEmpreendimentos.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformEmpreendimentos.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformEmpreendimentos.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformEmpreendimentos.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformEmpreendimentos.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformEmpreendimentos.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformEmpreendimentos.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
...
procedure TformEmpreendimentos.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformEmpreendimentos.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformEmpreendimentos.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformEmpreendimentos.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformEmpreendimentos.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformEmpreendimentos.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformEmpreendimentos.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformEmpreendimentos.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformEmpreendimentos.btnSalvarClick(Sender: TObject);
const
     myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformEmpreendimentos.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
end;

procedure TformEmpreendimentos.lblItensClick(Sender: TObject);
begin
   if qrySelecionar.Active then
    if not qrySelecionar.IsEmpty then
     with TformAreasEmp.Create(Self) do
      try
       inEmp := Self.qrySelecionar.fieldByName('CODIGO').AsInteger;
       SetWhereDynamic(Format('(CODEMP = %d)',
...
procedure TformEmpreendimentos.qryDetalhesAfterOpen(DataSet: TDataSet);
begin
  with Dataset do
...
procedure TformEmpreendimentos.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformEmpreendimentos.Grade1Click(Sender: TObject);
begin
...
procedure TformEmpreendimentos.RelatriodeClientesreas1Click(
  Sender: TObject);
begin
  ...
  ppLabel1.Caption := qrySelecionar.fieldByName('NOME').AsString;
  ppReport.Print;

end;

procedure TformEmpreendimentos.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformEmpreendimentos.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
