...
procedure TformOperadoras.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformOperadoras.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformOperadoras.FormShow(Sender: TObject);
var
  i: byte;
const
  myEnter: char = #13;
...
procedure TformOperadoras.btnSairClick(Sender: TObject);
begin
//     close;
end;

procedure TformOperadoras.FormCreate(Sender: TObject);
begin
...
procedure TformOperadoras.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformOperadoras.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformOperadoras.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformOperadoras.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformOperadoras.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformOperadoras.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformOperadoras.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformOperadoras.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformOperadoras.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
   qrySelecionar.tag := 0;
...
procedure TformOperadoras.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
  with Dataset do
   begin
...
procedure TformOperadoras.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformOperadoras.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformOperadoras.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformOperadoras.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformOperadoras.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformOperadoras.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformOperadoras.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformOperadoras.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformOperadoras.btnSalvarClick(Sender: TObject);
const
     myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformOperadoras.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
end;

procedure TformOperadoras.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformOperadoras.btnImprimirClick(Sender: TObject);
begin
...
procedure TformOperadoras.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformOperadoras.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [];
end;

procedure TformOperadoras.btnArquivosClick(Sender: TObject);
begin
   if qrySelecionar.Active then
    if not qrySelecionar.IsEmpty then
     with TformPlanos.Create(Self) do
      try
       inOperadora := Self.qrySelecionar.fieldByName('CODIGO').AsInteger;
