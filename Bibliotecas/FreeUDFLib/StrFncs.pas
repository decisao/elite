...
function TestaCGC(yCGC: PChar): Integer; cdecl; export;
function TestaCPF(yCPF: PChar): Integer; cdecl; export;
function Ean13(codi: pchar): pchar; cdecl; export;
...
function TestaCGC(yCGC: PChar):Integer;
Var
d1,d4,xx,nCount,fator,resto,digito1,digito2 : Integer;
Check, xCGC : String;
begin
  xCGC := '';
  for d1 := 0 to StrLen(yCGC) - 1 do
   if not (yCGC[d1] in [' ', '.', '-', '/']) then
     xCGC := xCGC + yCGC[d1];
  if xCGC = '' then
   begin
    Result := 1;
...
function TestaCPF( yCPF: pchar ): Integer;
Var
  d1,d4,xx,nCount,resto,digito1,digito2 : Integer;
  Check, xCPF : String;
Begin
  xCPF := '';
  for d1 := 0 to StrLen(yCPF) - 1 do
   if not (yCPF[d1] in [' ', '.', '-', '/']) then
     xCPF := xCPF + yCPF[d1];
  if xCPF = '' then
   begin
    Result := 1;
...
function Ean13(codi: pchar): pchar;
var
  soma: word;
  i: byte;
begin
  soma := 0;
  for i := 0 to 11 do
   if odd(i) then soma := soma + (ord(codi[i]) - 48) * 3 else
     soma := soma + (ord(codi[i]) - 48);
  soma := 10 - (soma mod 10);
  if soma = 10 then soma := 0;
  codi[12] := chr(soma + 48);
  Result := MakeResultString(codi, nil, 0);
end;
