...
procedure TformBancos.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformBancos.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformBancos.FormCreate(Sender: TObject);
begin
...
procedure TformBancos.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformBancos.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformBancos.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformBancos.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformBancos.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformBancos.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformBancos.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformBancos.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformBancos.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformBancos.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformBancos.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformBancos.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformBancos.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformBancos.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
end;

procedure TformBancos.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformBancos.btnImprimirClick(Sender: TObject);
begin
...
procedure TformBancos.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformBancos.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
