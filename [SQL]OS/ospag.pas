...
{######################################################################
 ...
 ######################################################################}
  ...
  WhereArgStatic = '(PAGAMENTOS.ES = 2) AND ' +
...
procedure TformOSpag.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformOSpag.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformOSpag.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformOSpag.FormCreate(Sender: TObject);
begin
...
procedure TformOSpag.edtSelecionarKeyPress(Sender: TObject);
var
  i, sel: byte;
  StrSQL: string;
begin
...
procedure TformOSpag.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformOSpag.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformOSpag.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformOSpag.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformOSpag.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformOSpag.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformOSpag.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformOSpag.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformOSpag.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformOSpag.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformOSpag.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformOSpag.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
     ...
     with qryDetalhes do
      begin
       ...
       btnExcluir.Enabled := (fieldByName('PAGO').AsInteger = 0) and
            (dmoPrincipal.cdsVendedor.fieldByName('FINANCEIRO').AsString = 'S');
       btnBaixa.Enabled := (fieldByName('PAGO').AsInteger = 0) and
            (dmoPrincipal.cdsVendedor.fieldByName('RECEBIMENTO').AsString = 'S');
      end;
     dbnDetalhes.BtnClick(nbEdit);
     Screen.Cursor := crDefault;
end;

procedure TformOSpag.qryDetalhesAfterOpen(DataSet: TDataSet);
begin
     with Dataset do
      begin
...
procedure TformOSpag.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformOSpag.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformOSpag.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformOSpag.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformOSpag.btnSalvarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformOSpag.FormDestroy(Sender: TObject);
begin
...
procedure TformOSpag.DBEdit2KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformOSpag.DBEdit1KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformOSpag.DBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformOSpag.DBEdit5KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformOSpag.DBEdit6KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformOSpag.btnBaixaClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
     dbnDetalhes.BtnClick(nbPost);
...
procedure TformOSpag.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformOSpag.Label13Click(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
...
procedure TformOSpag.fldCODDOCUMENTOmudar(Sender: TField);
begin
...
procedure TformOSpag.btnImprimirClick(Sender: TObject);
begin
...
procedure TformOSpag.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformOSpag.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
