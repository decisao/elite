...
type
  TformImprimirOSaberta = class(TForm)
    qryImpNota: TIBQuery;
...
var
  formImprimirOSaberta: TformImprimirOSaberta;
...
procedure TformImprimirOSaberta.ImprimeNota(Sender: TObject);
var
  Campo: TField;
...
begin
  ...
  if (Pos(uppercase(dmoPrincipal.NomeComputador),
     uppercase(dmoPrincipal.cdsConfig.fieldByName('PRINT_OS').AsString) ) > 0)
...
procedure TformImprimirOSaberta.Button1Click(Sender: TObject);
begin
...
procedure TformImprimirOSaberta.FormCreate(Sender: TObject);
var
  inI: Integer;
begin
...
procedure TformImprimirOSaberta.qryNotaCorpoBeforeOpen(DataSet: TDataSet);
begin
  with (DataSet as TIBQuery) do
    paramByName('CODMOVIMENTO').AsInteger := inCODMOVIMENTO;      
end;

procedure TformImprimirOSaberta.qryImpNotaBeforeOpen(DataSet: TDataSet);
begin
  with (DataSet as TIBQuery) do
...
procedure TformImprimirOSaberta.FormShow(Sender: TObject);
begin
...
procedure TformImprimirOSaberta.BitBtn1Click(Sender: TObject);
begin
   close;
end;

procedure TformImprimirOSaberta.qryCabNotaAfterOpen(DataSet: TDataSet);
begin
   cbCopias.ItemIndex :=
     pred(Dataset.fieldByName('COPIAS').AsInteger);
end;

procedure TformImprimirOSaberta.qryCabNotaBeforeOpen(DataSet: TDataSet);
begin
  with (Dataset as TIBQuery) do
