 ...
 ######################################################################}
  ...
  ixCampos  : array[1..ixMaximo] of TCampoDes =
     ( (rotulo: 'Código'        ; fisico: 'CODIGO'           ; iTab: 1),
       ...
       (rotulo: 'Nro. Série'    ; fisico: 'SERIE'            ; iTab: 1),
...
begin
     ...
     { se estiver fechado, somente o cancelar estará ativo }
...
var
  marca: TbookmarkStr;
begin
   if dmoPrincipal.Pergunta('#, se você estornar essa Saída, todos os produtos ' +
      'serão devolvidos, incluindo a movimentação financeira.' + #10 + #13 + #10 + #13 +
   ...
   begin
    ...
    if (not fieldByName('NUMERO_OS').IsNull) and
      ...
      if dmoPrincipal.Pergunta(
         Format('O cliente selecionado já possui OS em aberto!' + #13#10#13#10 +
           'Número da última OS: %d' + #13#10 +
           'Aberta em %s' + #13#10 +
           'Responsável pela abertura: %s' + #13#10 +
           'Solicitada por: %s' + #13#10#13#10 +
           'Existem outras OS em aberto além desta? %s' + #13#10#13#10 +
           'Deseja continuar fazendo a abertura atual?',
           [fieldByName('NUMERO_OS').AsInteger,
            formatDateTime('dddd, dd "de" mmmm "de" yyyy, "às" H"h"mm"min"',
               fieldByName('DATA').AsDateTime),
            fieldByName('RESPONSAVEL').AsString,
            fieldByName('SOLICITANTE').AsString,
            estatistica]),
         'Atenção, #!', 1) = IDNO then btnCancelarClick(nil);
   end;
end;
...
begin
     ...
     with TformClientes.Create(Self) do
      try
       ...
       { passo o código para o JumpStart }
     ...
     if showmodal = mrOk then
      begin
       ...
       if Application.MessageBox(PChar(Format(
          'Informações do equipamento escolhido: ' +#13#10#13#10+
          'Marca: %s' + #13#10+
          'Modelo: %s' + #13#10+
          'Série: %s' + #13#10+
          'Garantia: %s' + #13#10+
...
begin
  ...
  else if not primeira then ShowMessage('Nenhuma ocorrência encontrada.');
end;
    ...
    if trim(fieldByName('NOMEUSER').AsString) = dmoPrincipal.GetLogUser then
     begin
       ...
       fieldByName('DATAFIM').AsDateTime :=
          dmoPrincipal.GetServerTime;
       post;
     end else ShowMessage('Somente o técnico que iniciou a O.S. pode terminar!');
   end;
...
begin
  ...
  pplbEquipamento.Caption := 'EQUIPAMENTO: ' +
    ...
    trim(qryOSequip.fieldByName('MODELO').AsString) + ' - SÉRIE: ' +
    ...
    begin
      ...
      if fieldByName('QUEM_RECEBEU').AsString > '' then
       if dmoPrincipal.cdsVendedor.FieldByName('ESTORNO').AsString = 'N' then
        begin
          Application.MessageBox('Entrega já efetuada.', 'Entrega',
      ...
      if state in [dsEdit, dsInsert] then
       begin
         fieldByName('QUEM_RECEBEU').AsString := uppercase(
           InputBox('Entrega', 'Quem está recebendo o equipamento?', ''));
         if fieldByName('QUEM_RECEBEU').AsString > '' then
          begin
