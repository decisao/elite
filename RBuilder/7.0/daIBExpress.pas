...
{##############################################################################}
{                                                                              }
{ Modificado por Elieser Morais - GetTableNames add ttView                     }
{                                                                              }
{##############################################################################}
...
// old version - only tables and views by Elieser
...
{ new version - tables, views and procedures with no input parameters }
procedure TdaIBXSession.GetTableNames(const aDatabaseName: String; aList: TStrings);
var
  lDatabase: TIBDatabase;
  lQuery: TIBQuery;
...
begin
  ...
  { sql command is needed to recover tables, views and procedures names }
  lQuery.SQL.Add(
        'SELECT                             ' +
        '  TA.RDB$RELATION_NAME AS TABELA   ' +
        'FROM                               ' +
        '  RDB$RELATIONS TA                 ' +
        'UNION                              ' +
        'SELECT                             ' +
        '  PO.RDB$PROCEDURE_NAME AS TABELA  ' +
        'FROM                               ' +
        '  RDB$PROCEDURES PO                ' +
        'WHERE                              ' +
        '  PO.RDB$PROCEDURE_INPUTS IS NULL  ' +
        'ORDER BY                           ' +
        '  1                                ');
   ...
   begin
     ...
     with lQuery do
      begin
       ...
       while not eof do
        begin
         aList.Add(trim(fieldByName('TABELA').AsString));
         next;
        end;
      end;
   end;
...
begin
  ...
  if (lDatabase = nil) then
    raise EDataError('TdaIBXQueryDataView.SQLChanged: No IBDatabase object found, ' + SQL.DatabaseName);
