...
type
  TformImprimirOSfechada = class(TForm)
    qryImpNota: TIBQuery;
...
var
  formImprimirOSfechada: TformImprimirOSfechada;
...
procedure TformImprimirOSfechada.ImprimeNota(Sender: TObject);
var
  Campo: TField;
...
begin
  ...
  if (Pos(uppercase(dmoPrincipal.NomeComputador),
     uppercase(dmoPrincipal.cdsConfig.fieldByName('PRINT_OS2').AsString) ) > 0)
...
procedure TformImprimirOSfechada.Button1Click(Sender: TObject);
begin
...
procedure TformImprimirOSfechada.FormCreate(Sender: TObject);
var
  inI: Integer;
begin
...
procedure TformImprimirOSfechada.qryNotaCorpoBeforeOpen(DataSet: TDataSet);
begin
  with (DataSet as TIBQuery) do
    paramByName('CODMOVIMENTO').AsInteger := inCODMOVIMENTO;      
end;

procedure TformImprimirOSfechada.qryImpNotaBeforeOpen(DataSet: TDataSet);
begin
  with (DataSet as TIBQuery) do
...
procedure TformImprimirOSfechada.FormShow(Sender: TObject);
begin
...
procedure TformImprimirOSfechada.BitBtn1Click(Sender: TObject);
begin
  close;
end;

procedure TformImprimirOSfechada.qryCabNotaAfterOpen(DataSet: TDataSet);
begin
   cbCopias.ItemIndex :=
     pred(Dataset.fieldByName('COPIAS').AsInteger);
end;

procedure TformImprimirOSfechada.qryCabNotaBeforeOpen(DataSet: TDataSet);
begin
  with (Dataset as TIBQuery) do
