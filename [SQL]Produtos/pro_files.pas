...
procedure TformPro_files.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformPro_files.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformPro_files.FormCreate(Sender: TObject);
begin
...
procedure TformPro_files.edtSelecionarKeyPress(Sender: TObject);
var
  i, sel: byte;
  StrSQL: string;
begin
...
procedure TformPro_files.cmbOrdenarChange(Sender: TObject);
begin
     tmpSQLOrd := ' ORDER BY ' +
       ixCampos[succ(cmbOrdenar.ItemIndex)].fisico;
     with qrySelecionar do
...
procedure TformPro_files.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformPro_files.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformPro_files.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformPro_files.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformPro_files.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
  ListItem: TListItem;
...
procedure TformPro_files.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformPro_files.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformPro_files.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformPro_files.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformPro_files.FormDestroy(Sender: TObject);
begin
...
procedure TformPro_files.DBEdit2KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformPro_files.DBEdit1KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformPro_files.DBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformPro_files.DBEdit5KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformPro_files.DBEdit6KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformPro_files.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformPro_files.ToolbarButton971Click(Sender: TObject);
var
    ...
    begin
     ...
     with Params do
      begin
       ...
       byName('CODPRODUTO').AsInteger := inProduto;
...
procedure TformPro_files.ComboBox1Change(Sender: TObject);
begin
  with ComboBox1 do
    ListView1.ViewStyle := TViewStyle(Items.Objects[ItemIndex]);
end;

procedure TformPro_files.ListView1SelectItem(Sender: TObject;
...
procedure TformPro_files.btnExcluirClick(Sender: TObject);
begin
...
procedure TformPro_files.btnDownloadClick(Sender: TObject);
begin
  if ListView1.Selected = nil then exit;
...
procedure TformPro_files.ListView1Click(Sender: TObject);
var
  caminho: string;
begin
...
procedure TformPro_files.ToolbarButton971MouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformPro_files.ToolbarButton971MouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
