...
uses data_principal, cartas_perguntas, cartas_legendas;
 ...
 ######################################################################}
  ...
  ixCampos  : array[1..ixMaximo] of TCampoDes =
     ( (rotulo: 'Código'       ; fisico: 'CODIGO'   ; iTab: 1),
       (rotulo: 'Nome da Carta'; fisico: 'DESCRICAO'; iTab: 1) );
...
procedure TformCartas.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformCartas.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformCartas.btnSairClick(Sender: TObject);
begin
//     close;
end;
...
procedure TformCartas.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformCartas.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformCartas.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformCartas.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformCartas.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformCartas.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformCartas.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCartas.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCartas.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformCartas.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
    Dataset.fieldByName(ixCampos[1].fisico).AsInteger :=
      dmoPrincipal.LerSequencia(ixTabelas[1]);
end;

procedure TformCartas.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformCartas.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformCartas.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformCartas.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformCartas.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformCartas.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformCartas.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformCartas.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformCartas.btnSalvarClick(Sender: TObject);
const
     myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformCartas.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
end;

procedure TformCartas.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformCartas.btnImprimirClick(Sender: TObject);
begin
...
procedure TformCartas.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformCartas.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [];
end;

procedure TformCartas.btnArquivosClick(Sender: TObject);
begin
   if qrySelecionar.Active then
    if not qrySelecionar.IsEmpty then
     with TformCartas_perguntas.Create(Self) do
      try
       inCarta := Self.qrySelecionar.fieldByName('CODIGO').AsInteger;
       SetWhereDynamic(Format('CARTAS_PERGUNTA.CODCARTA = %d',
        [Self.qrySelecionar.fieldByName('CODIGO').AsInteger]),
        Format('Perguntas da Carta %d - %s',
          [Self.qrySelecionar.fieldByName('CODIGO').AsInteger,
...
procedure TformCartas.btnHistoricoClick(Sender: TObject);
begin
   if qrySelecionar.Active then
    if not qrySelecionar.IsEmpty then
     with TformCartas_legendas.Create(Self) do
      try
       inCarta := Self.qrySelecionar.fieldByName('CODIGO').AsInteger;
       SetWhereDynamic(Format('CARTAS_LEGENDA.CODCARTA = %d',
        [Self.qrySelecionar.fieldByName('CODIGO').AsInteger]),
        Format('Legendas da Carta %d - %s',
          [Self.qrySelecionar.fieldByName('CODIGO').AsInteger,
          Self.qrySelecionar.fieldByName('DESCRICAO').AsString]));
       Caption := Self.Caption + ' -> ' + Caption;
       ShowModal;
