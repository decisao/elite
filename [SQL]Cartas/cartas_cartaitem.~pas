...
type
  TformCartas_CartaItem = class(TForm)
...
var
  formCartas_CartaItem: TformCartas_CartaItem;
...
procedure TformCartas_CartaItem.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformCartas_CartaItem.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformCartas_CartaItem.FormShow(Sender: TObject);
var
  i: byte;
const
  myEnter: char = #13;
...
procedure TformCartas_CartaItem.btnSairClick(Sender: TObject);
begin
//     close;
end;

procedure TformCartas_CartaItem.FormCreate(Sender: TObject);
begin
...
procedure TformCartas_CartaItem.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformCartas_CartaItem.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformCartas_CartaItem.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformCartas_CartaItem.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformCartas_CartaItem.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformCartas_CartaItem.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformCartas_CartaItem.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCartas_CartaItem.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCartas_CartaItem.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
   qrySelecionar.tag := 0;
...
procedure TformCartas_CartaItem.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
  with Dataset do
   begin
...
procedure TformCartas_CartaItem.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformCartas_CartaItem.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformCartas_CartaItem.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformCartas_CartaItem.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformCartas_CartaItem.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformCartas_CartaItem.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformCartas_CartaItem.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformCartas_CartaItem.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformCartas_CartaItem.btnSalvarClick(Sender: TObject);
const
     myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformCartas_CartaItem.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
end;

procedure TformCartas_CartaItem.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformCartas_CartaItem.btnImprimirClick(Sender: TObject);
begin
...
procedure TformCartas_CartaItem.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformCartas_CartaItem.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
