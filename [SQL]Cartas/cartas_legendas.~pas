...
type
  TformCartas_legendas = class(TForm)
...
var
  formCartas_legendas: TformCartas_legendas;
...
procedure TformCartas_legendas.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformCartas_legendas.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformCartas_legendas.FormShow(Sender: TObject);
var
  i: byte;
const
  myEnter: char = #13;
...
procedure TformCartas_legendas.btnSairClick(Sender: TObject);
begin
//     close;
end;

procedure TformCartas_legendas.FormCreate(Sender: TObject);
begin
...
procedure TformCartas_legendas.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformCartas_legendas.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformCartas_legendas.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformCartas_legendas.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformCartas_legendas.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformCartas_legendas.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformCartas_legendas.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCartas_legendas.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCartas_legendas.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
   qrySelecionar.tag := 0;
...
procedure TformCartas_legendas.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
  with Dataset do
   begin
    fieldByname('CODCARTA').AsInteger := inCarta;
    fieldByName('CODLEGENDA').AsInteger := 0;
   end
end;

procedure TformCartas_legendas.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformCartas_legendas.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformCartas_legendas.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformCartas_legendas.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformCartas_legendas.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformCartas_legendas.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformCartas_legendas.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformCartas_legendas.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformCartas_legendas.btnSalvarClick(Sender: TObject);
const
     myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformCartas_legendas.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
end;

procedure TformCartas_legendas.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformCartas_legendas.btnImprimirClick(Sender: TObject);
begin
...
procedure TformCartas_legendas.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformCartas_legendas.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
