...
type
  TformCartas_perguntas = class(TForm)
...
var
  formCartas_perguntas: TformCartas_perguntas;
...
procedure TformCartas_perguntas.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformCartas_perguntas.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformCartas_perguntas.FormShow(Sender: TObject);
var
  i: byte;
const
  myEnter: char = #13;
...
procedure TformCartas_perguntas.btnSairClick(Sender: TObject);
begin
//     close;
end;

procedure TformCartas_perguntas.FormCreate(Sender: TObject);
begin
...
procedure TformCartas_perguntas.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformCartas_perguntas.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformCartas_perguntas.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformCartas_perguntas.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformCartas_perguntas.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformCartas_perguntas.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformCartas_perguntas.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCartas_perguntas.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCartas_perguntas.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
   qrySelecionar.tag := 0;
...
procedure TformCartas_perguntas.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
  with Dataset do
   begin
...
procedure TformCartas_perguntas.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformCartas_perguntas.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformCartas_perguntas.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformCartas_perguntas.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformCartas_perguntas.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformCartas_perguntas.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformCartas_perguntas.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformCartas_perguntas.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformCartas_perguntas.btnSalvarClick(Sender: TObject);
const
     myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformCartas_perguntas.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
end;

procedure TformCartas_perguntas.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformCartas_perguntas.btnImprimirClick(Sender: TObject);
begin
...
procedure TformCartas_perguntas.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformCartas_perguntas.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
