...
type
  ...
  public
    { Public declarations }
    inCliente, inEquipamento: longint;
    procedure SetWhereDynamic(st, stmsg: string);
  end;

var
  formCartas_Carta: TformCartas_Carta;
...
{######################################################################
 ...
 ######################################################################}
  ...
  ixCampos  : array[1..ixMaximo] of TCampoDes =
     ( (rotulo: 'Cód.OS'     ; fisico: 'CODMOVIMENTO'  ; iTab: 1),
       (rotulo: 'Data'       ; fisico: 'DATA'          ; iTab: 1),
       (rotulo: 'Carta'      ; fisico: 'DESCRICAO'     ; iTab: 2),
       (rotulo: 'Cód.Cliente'; fisico: 'CODCLIENTE'    ; iTab: 1),
       (rotulo: 'Cód.Equip'  ; fisico: 'CODEQUIPAMENTO'; iTab: 1),
       (rotulo: 'Cód.Carta'  ; fisico: 'CODCARTA'      ; iTab: 1) );

  { expressão adicional do where - não pode ser mudada em run-time }
  { para acrescentar parâmentos em run-time use SetWhereDynamic!   }
  WhereArgStatic = 'CARTAS.CODIGO = CARTAS_OS.CODCARTA';
...
procedure TformCartas_Carta.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformCartas_Carta.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformCartas_Carta.FormShow(Sender: TObject);
var
  i: byte;
const
  myEnter: char = #13;
...
procedure TformCartas_Carta.btnSairClick(Sender: TObject);
begin
//     close;
end;

procedure TformCartas_Carta.FormCreate(Sender: TObject);
begin
...
procedure TformCartas_Carta.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformCartas_Carta.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformCartas_Carta.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformCartas_Carta.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformCartas_Carta.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformCartas_Carta.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformCartas_Carta.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCartas_Carta.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCartas_Carta.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
   qrySelecionar.tag := 0;
   ...
   grdDados.Columns[4].Width := 0;
   grdDados.Columns[5].Width := 0;
end;

procedure TformCartas_Carta.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
//  with Dataset do
//   begin
//    fieldByname('CODCARTA').AsInteger := inCarta;
//    fieldByName('CODPERGUNTA').AsInteger := 0;
//    fieldByName('TIPO').AsString := 'L';
//   end
end;

procedure TformCartas_Carta.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformCartas_Carta.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformCartas_Carta.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformCartas_Carta.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformCartas_Carta.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformCartas_Carta.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformCartas_Carta.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformCartas_Carta.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformCartas_Carta.btnSalvarClick(Sender: TObject);
const
     myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformCartas_Carta.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
end;

procedure TformCartas_Carta.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformCartas_Carta.btnImprimirClick(Sender: TObject);
begin
...
procedure TformCartas_Carta.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformCartas_Carta.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
