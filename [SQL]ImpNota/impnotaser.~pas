...
var
  formImpNotaSer: TformImpNotaSer;

implementation

uses data_principal, paramNFser;
...
procedure TformImpNotaSer.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformImpNotaSer.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformImpNotaSer.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformImpNotaSer.FormCreate(Sender: TObject);
begin
...
procedure TformImpNotaSer.edtSelecionarKeyPress(Sender: TObject);
var
  i, sel: byte;
  StrSQL: string;
begin
...
procedure TformImpNotaSer.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformImpNotaSer.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformImpNotaSer.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformImpNotaSer.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformImpNotaSer.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformImpNotaSer.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformImpNotaSer.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformImpNotaSer.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformImpNotaSer.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
    { número da nota pai }
    with Dataset do
     begin
...
procedure TformImpNotaSer.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformImpNotaSer.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformImpNotaSer.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformImpNotaSer.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformImpNotaSer.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformImpNotaSer.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformImpNotaSer.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformImpNotaSer.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformImpNotaSer.btnSalvarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformImpNotaSer.FormDestroy(Sender: TObject);
begin
...
procedure TformImpNotaSer.DBEdit2KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpNotaSer.DBEdit1KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpNotaSer.DBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpNotaSer.DBEdit5KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpNotaSer.DBEdit6KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpNotaSer.lblREAJUSTEClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with TformParamNFSer.Create(Self) do
      try
...
procedure TformImpNotaSer.DBEdit3Enter(Sender: TObject);
begin
     if Sender is TEdit then
      begin
       TEdit(Sender).Color := clInfoBK;
...
procedure TformImpNotaSer.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformImpNotaSer.btnImprimirClick(Sender: TObject);
begin
...
procedure TformImpNotaSer.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformImpNotaSer.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
