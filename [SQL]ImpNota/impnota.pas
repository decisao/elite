...
var
  formImpNota: TformImpNota;

implementation

uses data_principal, impnotacorpo, impnotadup, impnotapro, impnotaser;
...
{######################################################################
 ...
 ######################################################################}
  ...
  ixCampos  : array[1..ixMaximo] of TCampoDes =
     ( (rotulo: 'Código'         ; fisico: 'NUMIMPNOTA'; iTab: 1),
...
procedure TformImpNota.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformImpNota.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformImpNota.FormShow(Sender: TObject);
var
  i: byte;
  tecla: char;
begin
...
procedure TformImpNota.FormCreate(Sender: TObject);
begin
     inProduto := 0;
...
procedure TformImpNota.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformImpNota.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformImpNota.cmbSelecionarChange(Sender: TObject);
begin
     edtSelecionar.Clear;
     edtSelecionar.SetFocus;
end;

procedure TformImpNota.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformImpNota.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformImpNota.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformImpNota.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformImpNota.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformImpNota.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformImpNota.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformImpNota.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
...
procedure TformImpNota.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformImpNota.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformImpNota.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformImpNota.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformImpNota.FormCloseQuery(Sender: TObject;
...
procedure TformImpNota.fldPRECOCUSTOmudar(Sender: TField);
var
...
procedure TformImpNota.fldPRECOVENDAmudar(Sender: TField);
var
  custo, venda, mora: extended;
begin
  mora  := 0.0;
...
procedure TformImpNota.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformImpNota.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformImpNota.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformImpNota.btnSalvarClick(Sender: TObject);
const
     myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformImpNota.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
end;

procedure TformImpNota.lblMENSAGEMClick(Sender: TObject);
begin
   Screen.Cursor := crHourGlass;  
   if qrySelecionar.Active then
    if not qrySelecionar.IsEmpty  then
     with TformImpNotaCorpo.Create(Self) do
      try
       inNF := Self.qrySelecionar.fieldByName('NUMIMPNOTA').AsInteger;
       SetWhereDynamic(Format('(NUMIMPNOTA = %d)',
        [Self.qrySelecionar.fieldByName('NUMIMPNOTA').AsInteger]),
        Format('Corpo da NF %d - %s',
          [Self.qrySelecionar.fieldByName('NUMIMPNOTA').AsInteger,
...
procedure TformImpNota.lblREAJUSTEClick(Sender: TObject);
begin
   Screen.Cursor := crHourGlass;
   if qrySelecionar.Active then
    if not qrySelecionar.IsEmpty  then
     with TformImpNotaDup.Create(Self) do
      try
       inNF := Self.qrySelecionar.fieldByName('NUMIMPNOTA').AsInteger;
       SetWhereDynamic(Format('(NUMIMPNOTA = %d)',
        [Self.qrySelecionar.fieldByName('NUMIMPNOTA').AsInteger]),
        Format('Duplicatas da NF %d - %s',
          [Self.qrySelecionar.fieldByName('NUMIMPNOTA').AsInteger,
          Self.qrySelecionar.fieldByName('DESCRICAO').AsString]));
       Caption := Self.Caption + ' -> ' + Caption;
       ShowModal;
...
procedure TformImpNota.DBEdit19KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);
end;

procedure TformImpNota.DBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpNota.DBEdit4KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpNota.Label13Click(Sender: TObject);
begin
   Screen.Cursor := crHourGlass;
   if qrySelecionar.Active then
    if not qrySelecionar.IsEmpty  then
     with TformImpNotaPro.Create(Self) do
      try
       inNF := Self.qrySelecionar.fieldByName('NUMIMPNOTA').AsInteger;
       SetWhereDynamic(Format('(NUMIMPNOTA = %d)',
        [Self.qrySelecionar.fieldByName('NUMIMPNOTA').AsInteger]),
        Format('Produtos da NF %d - %s',
          [Self.qrySelecionar.fieldByName('NUMIMPNOTA').AsInteger,
          Self.qrySelecionar.fieldByName('DESCRICAO').AsString]));
       Caption := Self.Caption + ' -> ' + Caption;
       ShowModal;
...
procedure TformImpNota.Label17Click(Sender: TObject);
begin
   Screen.Cursor := crHourGlass;
   if qrySelecionar.Active then
    if not qrySelecionar.IsEmpty  then
     with TformImpNotaSer.Create(Self) do
      try
       inNF := Self.qrySelecionar.fieldByName('NUMIMPNOTA').AsInteger;
       SetWhereDynamic(Format('(NUMIMPNOTA = %d)',
        [Self.qrySelecionar.fieldByName('NUMIMPNOTA').AsInteger]),
        Format('Serviços da NF %d - %s',
          [Self.qrySelecionar.fieldByName('NUMIMPNOTA').AsInteger,
          Self.qrySelecionar.fieldByName('DESCRICAO').AsString]));
       Caption := Self.Caption + ' -> ' + Caption;
       ShowModal;
...
procedure TformImpNota.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformImpNota.btnImprimirClick(Sender: TObject);
begin
...
procedure TformImpNota.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformImpNota.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
