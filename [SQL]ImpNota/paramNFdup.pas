...
var
  formParamNFDup: TformParamNFDup;
...
{######################################################################
 ...
 ######################################################################}
  ...
  ixTabelas : array[1..tblCount] of TCampoStr =
     ( 'PROC_PARAMS(''REL_NOTADUP'')' );

  { entrada no registro do Windows }
  NomeCurto = 'paramNFdup';
...
procedure TformParamNFDup.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformParamNFDup.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformParamNFDup.FormCreate(Sender: TObject);
begin
...
procedure TformParamNFDup.edtSelecionarKeyPress(Sender: TObject);
var
  i, sel: byte;
  StrSQL: string;
begin
...
procedure TformParamNFDup.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixCampos[succ(cmbOrdenar.ItemIndex)].fisico;
...
procedure TformParamNFDup.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformParamNFDup.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformParamNFDup.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformParamNFDup.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformParamNFDup.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformParamNFDup.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformParamNFDup.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformParamNFDup.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformParamNFDup.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformParamNFDup.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformParamNFDup.FormDestroy(Sender: TObject);
begin
...
procedure TformParamNFDup.lblItensClick(Sender: TObject);
begin
...
procedure TformParamNFDup.DBEdit2KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFDup.DBEdit1KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFDup.DBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFDup.DBEdit5KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFDup.DBEdit6KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFDup.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformParamNFDup.btnSairMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformParamNFDup.btnSairMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
