...
var
  formImpNotaPro: TformImpNotaPro;

implementation

uses data_principal, paramNFpro;
...
procedure TformImpNotaPro.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformImpNotaPro.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformImpNotaPro.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformImpNotaPro.FormCreate(Sender: TObject);
begin
...
procedure TformImpNotaPro.edtSelecionarKeyPress(Sender: TObject);
var
  i, sel: byte;
  StrSQL: string;
begin
...
procedure TformImpNotaPro.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformImpNotaPro.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformImpNotaPro.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformImpNotaPro.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformImpNotaPro.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformImpNotaPro.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformImpNotaPro.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformImpNotaPro.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformImpNotaPro.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
    { número da nota pai }
    with Dataset do
     begin
...
procedure TformImpNotaPro.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformImpNotaPro.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformImpNotaPro.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformImpNotaPro.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformImpNotaPro.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformImpNotaPro.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformImpNotaPro.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformImpNotaPro.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformImpNotaPro.btnSalvarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformImpNotaPro.FormDestroy(Sender: TObject);
begin
...
procedure TformImpNotaPro.DBEdit2KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpNotaPro.DBEdit1KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpNotaPro.DBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpNotaPro.DBEdit5KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpNotaPro.DBEdit6KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpNotaPro.lblREAJUSTEClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with TformParamNFPro.Create(Self) do
      try
...
procedure TformImpNotaPro.DBEdit3Enter(Sender: TObject);
begin
     if Sender is TEdit then
      begin
       TEdit(Sender).Color := clInfoBK;
...
procedure TformImpNotaPro.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformImpNotaPro.btnImprimirClick(Sender: TObject);
begin
...
procedure TformImpNotaPro.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformImpNotaPro.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
