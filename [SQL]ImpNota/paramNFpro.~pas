...
var
  formParamNFPro: TformParamNFPro;
...
{######################################################################
 ...
 ######################################################################}
  ...
  ixTabelas : array[1..tblCount] of TCampoStr =
     ( 'PROC_PARAMS(''REL_NOTAPRO'')' );

  { entrada no registro do Windows }
  NomeCurto = 'paramNFpro';
...
procedure TformParamNFPro.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformParamNFPro.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformParamNFPro.FormCreate(Sender: TObject);
begin
...
procedure TformParamNFPro.edtSelecionarKeyPress(Sender: TObject);
var
  i, sel: byte;
  StrSQL: string;
begin
...
procedure TformParamNFPro.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixCampos[succ(cmbOrdenar.ItemIndex)].fisico;
...
procedure TformParamNFPro.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformParamNFPro.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformParamNFPro.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformParamNFPro.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformParamNFPro.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformParamNFPro.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformParamNFPro.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformParamNFPro.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformParamNFPro.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformParamNFPro.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformParamNFPro.FormDestroy(Sender: TObject);
begin
...
procedure TformParamNFPro.lblItensClick(Sender: TObject);
begin
...
procedure TformParamNFPro.DBEdit2KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFPro.DBEdit1KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFPro.DBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFPro.DBEdit5KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFPro.DBEdit6KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFPro.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformParamNFPro.btnSairMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformParamNFPro.btnSairMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
