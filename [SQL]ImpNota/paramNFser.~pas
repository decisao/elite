...
var
  formParamNFSer: TformParamNFSer;
...
{######################################################################
 ...
 ######################################################################}
  ...
  ixTabelas : array[1..tblCount] of TCampoStr =
     ( 'PROC_PARAMS(''REL_NOTASER'')' );

  { entrada no registro do Windows }
  NomeCurto = 'paramNFser';
...
procedure TformParamNFSer.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformParamNFSer.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformParamNFSer.FormCreate(Sender: TObject);
begin
...
procedure TformParamNFSer.edtSelecionarKeyPress(Sender: TObject);
var
  i, sel: byte;
  StrSQL: string;
begin
...
procedure TformParamNFSer.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixCampos[succ(cmbOrdenar.ItemIndex)].fisico;
...
procedure TformParamNFSer.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformParamNFSer.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformParamNFSer.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformParamNFSer.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformParamNFSer.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformParamNFSer.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformParamNFSer.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformParamNFSer.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformParamNFSer.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformParamNFSer.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformParamNFSer.FormDestroy(Sender: TObject);
begin
...
procedure TformParamNFSer.lblItensClick(Sender: TObject);
begin
...
procedure TformParamNFSer.DBEdit2KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFSer.DBEdit1KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFSer.DBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFSer.DBEdit5KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFSer.DBEdit6KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformParamNFSer.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformParamNFSer.btnSairMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformParamNFSer.btnSairMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
