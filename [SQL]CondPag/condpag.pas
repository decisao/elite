...
type
  TformCondPag = class(TForm)
    ...
    traDOCprazo: TIBTransaction;
    traDOCvista: TIBTransaction;
    traCOND: TIBTransaction;
    qryCOND: TIBQuery;
  ...
  private
    ...
    procedure fldCONDICAOmudar(Sender: TField);
  public
    { Public declarations }
...
var
  formCondPag: TformCondPag;

implementation

uses data_principal, documentos;
 ...
 ######################################################################}
  ...
  ixCampos  : array[1..ixMaximo] of TCampoDes =
     ( (rotulo: 'Código'   ; fisico: 'CODIGO'    ; iTab: 1),
       ...
       (rotulo: 'Parcelas' ; fisico: 'QUANTIDADE'; iTab: 1),
       (rotulo: 'Juros'    ; fisico: 'TAXAJUROS' ; iTab: 1));
...
procedure TformCondPag.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformCondPag.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformCondPag.FormShow(Sender: TObject);
var
  i: byte;
const
  myEnter: char = #13;
...
procedure TformCondPag.btnSairClick(Sender: TObject);
begin
//     close;
end;

procedure TformCondPag.FormCreate(Sender: TObject);
begin
...
procedure TformCondPag.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformCondPag.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformCondPag.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformCondPag.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformCondPag.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformCondPag.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformCondPag.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCondPag.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCondPag.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
   qrySelecionar.tag := 0;
...
procedure TformCondPag.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
  with Dataset do
   begin
    fieldByName(ixCampos[1].fisico).AsInteger :=
      dmoPrincipal.LerSequencia('CONDPAG');

    fieldByname('CONDICAO').AsString := '0/';
    fieldByName('TAXAJUROS').AsCurrency := 0;
   end
end;

procedure TformCondPag.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformCondPag.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformCondPag.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformCondPag.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformCondPag.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformCondPag.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformCondPag.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformCondPag.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformCondPag.btnSalvarClick(Sender: TObject);
const
     myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformCondPag.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
end;

procedure TformCondPag.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformCondPag.btnImprimirClick(Sender: TObject);
begin
...
procedure TformCondPag.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformCondPag.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [];
end;

procedure TformCondPag.Label9Click(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
...
procedure TformCondPag.Label10Click(Sender: TObject);
begin
   Screen.Cursor := crHourGlass;
   with TformDocumentos.Create(Self) do
    try
...
procedure TformCondPag.fldCODDOC_AVISTAmudar(Sender: TField);
begin
...
procedure TformCondPag.fldCODDOC_PRAZOmudar(Sender: TField);
begin
  if Sender.AsString > '' then
   with qryDOCprazo do
    begin
...
procedure TformCondPag.fldCONDICAOmudar(Sender: TField);
begin
  if Sender.AsString > '' then
   with qryCOND do
    begin
     ...
     if Sender.DataSet.State in [dsInsert, dsEdit] then
      begin
        Sender.DataSet.FieldByName('QUANTIDADE').AsInteger :=
          FieldByName('QUANTIDADE').AsInteger;
        Sender.DataSet.FieldByName('PRAZOMEDIO').AsInteger :=
          FieldByName('PRAZOMEDIO').AsInteger;
      end;
    end;
end;
...
procedure TformCondPag.qryDetalhesAfterOpen(DataSet: TDataSet);
begin
  with Dataset do
   begin
     (fieldByName('TAXAJUROS') as TNumericField).DisplayFormat := '##0.0000 ''%''';
     (fieldByName('TAXAJUROS') as TNumericField).EditFormat := '##0.0000';

     (fieldByName('PRAZOMEDIO') as TNumericField).DisplayFormat := '##0 ''dias''';

     fieldByName('CODDOC_AVISTA').OnChange := fldCODDOC_AVISTAmudar;
     fieldByName('CODDOC_PRAZO').OnChange  := fldCODDOC_PRAZOmudar;
     fieldByName('CONDICAO').OnChange      := fldCONDICAOmudar;

     fldCODDOC_AVISTAmudar(fieldByName('CODDOC_AVISTA'));
     fldCODDOC_PRAZOmudar(fieldByName('CODDOC_PRAZO'));
   end;
