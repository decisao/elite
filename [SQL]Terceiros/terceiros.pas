...
uses
  ...
  TB97Ctls, Mailit, IBUpdateSQL, IBDatabase, gridreport;

type
  TformTerceiros = class(TForm)
...
procedure TformTerceiros.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformTerceiros.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformTerceiros.FormShow(Sender: TObject);
var
  i: byte;
  tecla: char;
begin
...
procedure TformTerceiros.FormCreate(Sender: TObject);
begin
     inFornecedor := 0;
...
procedure TformTerceiros.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformTerceiros.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformTerceiros.cmbSelecionarChange(Sender: TObject);
begin
     edtSelecionar.Clear;
     edtSelecionar.SetFocus;
end;

procedure TformTerceiros.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformTerceiros.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformTerceiros.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformTerceiros.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformTerceiros.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformTerceiros.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformTerceiros.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformTerceiros.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
...
procedure TformTerceiros.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformTerceiros.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformTerceiros.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformTerceiros.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformTerceiros.qryDetalhesAfterOpen(DataSet: TDataSet);
begin
     { mascaras }
     with Dataset do
      begin
...
procedure TformTerceiros.dbrPessoaChange(Sender: TObject);
begin
...
procedure TformTerceiros.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformTerceiros.fldESTADOmudar(Sender: TField);
begin
...
procedure TformTerceiros.fldCIDADEmudar(Sender: TField);
begin
  if Sender.AsString > '' then
...
procedure TformTerceiros.fldLOCALIDADEmudar(Sender: TField);
begin
  if Sender.AsString > '' then
...
procedure TformTerceiros.fldBAIRROmudar(Sender: TField);
begin
  if Sender.AsString > '' then
...
procedure TformTerceiros.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformTerceiros.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformTerceiros.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformTerceiros.btnSalvarClick(Sender: TObject);
const
     myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformTerceiros.lblESTADOClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
...
procedure TformTerceiros.FormDestroy(Sender: TObject);
begin
...
procedure TformTerceiros.lblCIDADEClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
...
procedure TformTerceiros.lblLOCALIZACAOClick(Sender: TObject);
var
  nova: boolean;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformTerceiros.CheckCPF1Error(Sender: TObject);
begin
     ShowMessage('O CPF digitado não é um CPF válido.');
end;

procedure TformTerceiros.CheckCGC1Error(Sender: TObject);
begin
     ShowMessage('O CNPJ digitado não é um CNPJ válido.');
end;

procedure TformTerceiros.DBEdit17Enter(Sender: TObject);
begin
     if Sender is TEdit then
      begin
       TEdit(Sender).Color := clInfoBK;
...
procedure TformTerceiros.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformTerceiros.qrySelecionarAfterScroll(DataSet: TDataSet);
begin
   if not Dataset.fieldByName('EMAIL').IsNull then
    begin
     MailIt.MailTo      := Trim(Dataset.fieldByName('EMAIL').AsString);
     MailIt.MailSubject := 'digite o assunto aqui';
     MailIt.Visible     := True;
    end else MailIt.Visible := False;
end;

procedure TformTerceiros.btnImprimirClick(Sender: TObject);
begin
