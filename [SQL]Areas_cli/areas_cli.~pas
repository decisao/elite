...
{######################################################################
 ...
 ######################################################################}
  ...
  WhereArgStatic = '(AREASCLI.CODBAIRRO = BAIRROS.CODIGO AND ' +
    'AREASCLI.CODCIDADE = BAIRROS.CIDADE AND ' +
    'AREASCLI.ESTADO = BAIRROS.ESTADO) AND ' +
    '(AREASCLI.CODCIDADE = CIDADES.CODIGO AND ' +
    'AREASCLI.ESTADO = CIDADES.ESTADO)';
...
procedure TformAreasCli.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformAreasCli.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformAreasCli.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformAreasCli.FormCreate(Sender: TObject);
begin
...
procedure TformAreasCli.edtSelecionarKeyPress(Sender: TObject);
var
  i, sel: byte;
  StrSQL: string;
begin
...
procedure TformAreasCli.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformAreasCli.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformAreasCli.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformAreasCli.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformAreasCli.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformAreasCli.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformAreasCli.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformAreasCli.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformAreasCli.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformAreasCli.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformAreasCli.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformAreasCli.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
     ...
     with qryDetalhes do
      begin
       if active then close; 
       ParamByName('CODCLI').AsInteger := inCLI;
...
procedure TformAreasCli.qryDetalhesAfterOpen(DataSet: TDataSet);
begin
     with Dataset do
      begin
...
procedure TformAreasCli.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformAreasCli.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformAreasCli.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformAreasCli.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformAreasCli.btnSalvarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformAreasCli.FormDestroy(Sender: TObject);
begin
...
procedure TformAreasCli.DBEdit2KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformAreasCli.DBEdit1KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformAreasCli.DBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformAreasCli.DBEdit5KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformAreasCli.DBEdit6KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformAreasCli.lblESTADOClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
...
procedure TformAreasCli.lblCIDADEClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
...
procedure TformAreasCli.lblBAIRROClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
...
procedure TformAreasCli.fldESTADOmudar(Sender: TField);
begin
...
procedure TformAreasCli.fldCIDADEmudar(Sender: TField);
begin
  if Sender.AsString > '' then
...
procedure TformAreasCli.fldBAIRROmudar(Sender: TField);
begin
  if Sender.AsString > '' then
   begin
...
procedure TformAreasCli.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
    Dataset.fieldByName('CODCLI').AsInteger := inCLI;
...
procedure TformAreasCli.DBEdit5Enter(Sender: TObject);
begin
     if Sender is TEdit then
      begin
       TEdit(Sender).Color := clInfoBK;
...
procedure TformAreasCli.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformAreasCli.btnImprimirClick(Sender: TObject);
begin
...
procedure TformAreasCli.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformAreasCli.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
