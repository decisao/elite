...
type
  TformImpBoletoCorpo = class(TForm)
...
var
  formImpBoletoCorpo: TformImpBoletoCorpo;

implementation

uses data_principal, paramNFdup;
...
procedure TformImpBoletoCorpo.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformImpBoletoCorpo.btnAdicionarClick(Sender: TObject);
begin
     ...
     with qryDetalhes do
      begin
       DisableControls;
       ParamByName('NUMIMPBOLETO').Clear;
...
procedure TformImpBoletoCorpo.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformImpBoletoCorpo.FormCreate(Sender: TObject);
begin
...
procedure TformImpBoletoCorpo.edtSelecionarKeyPress(Sender: TObject);
var
  i, sel: byte;
  StrSQL: string;
begin
...
procedure TformImpBoletoCorpo.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY LINHA, COLUNA';
     with qrySelecionar do
...
procedure TformImpBoletoCorpo.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformImpBoletoCorpo.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformImpBoletoCorpo.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformImpBoletoCorpo.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformImpBoletoCorpo.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformImpBoletoCorpo.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformImpBoletoCorpo.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformImpBoletoCorpo.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
    { número da nota pai }
    with Dataset do
     begin
      fieldByName('NUMIMPBOLETO').AsInteger := inNF;
      fieldByName('ALINHAMENTO').AsString := 'E';
      fieldByName('ALTURA').AsInteger := 1;
     end;
end;

procedure TformImpBoletoCorpo.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformImpBoletoCorpo.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformImpBoletoCorpo.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformImpBoletoCorpo.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
     ...
     with qryDetalhes do
      begin
       ParamByName('NUMIMPBOLETO').AsInteger :=
...
procedure TformImpBoletoCorpo.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformImpBoletoCorpo.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformImpBoletoCorpo.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformImpBoletoCorpo.btnSalvarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformImpBoletoCorpo.FormDestroy(Sender: TObject);
begin
...
procedure TformImpBoletoCorpo.DBEdit2KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpBoletoCorpo.DBEdit1KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpBoletoCorpo.DBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpBoletoCorpo.DBEdit5KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpBoletoCorpo.DBEdit6KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformImpBoletoCorpo.lblREAJUSTEClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with TformParamNFDup.Create(Self) do
      try
...
procedure TformImpBoletoCorpo.DBEdit3Enter(Sender: TObject);
begin
     if Sender is TEdit then
      begin
       TEdit(Sender).Color := clInfoBK;
...
procedure TformImpBoletoCorpo.DBEdit3Exit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
     ...
     if qryDetalhes.fieldByname('TAMANHO').IsNull and
      ...
      qryDetalhes.fieldByName('TAMANHO').AsInteger :=
       length(DBEdit3.Text) - 2;

end;

procedure TformImpBoletoCorpo.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
       TEdit(Sender).Color := clWindow;
...
procedure TformImpBoletoCorpo.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformImpBoletoCorpo.btnImprimirClick(Sender: TObject);
begin
...
procedure TformImpBoletoCorpo.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformImpBoletoCorpo.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
