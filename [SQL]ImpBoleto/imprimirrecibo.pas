...
type
  TformImprimirRecibo = class(TForm)
    qryImpNota: TIBQuery;
...
var
  formImprimirRecibo: TformImprimirRecibo;
...
procedure TformImprimirRecibo.ImprimeNota(Sender: TObject);
var
  Campo: TField;
...
begin
  ...
  if (Pos(uppercase(dmoPrincipal.NomeComputador),
     uppercase(dmoPrincipal.cdsConfig.fieldByName('PRINT_RECIBO').AsString) ) > 0)
...
procedure TformImprimirRecibo.Button1Click(Sender: TObject);
begin
...
procedure TformImprimirRecibo.FormCreate(Sender: TObject);
var
  inI: Integer;
begin
...
procedure TformImprimirRecibo.qryNotaCorpoBeforeOpen(DataSet: TDataSet);
begin
  with (DataSet as TIBQuery) do
   begin
    paramByName('CODMOVIMENTO').AsInteger := inCODMOVIMENTO;
    paramByName('CODIGO').AsInteger       := inNUMERONOTA;
   end;

end;

procedure TformImprimirRecibo.qryImpNotaBeforeOpen(DataSet: TDataSet);
begin
  with (DataSet as TIBQuery) do
   begin
    paramByName('NUMIMPBOLETO').AsInteger :=
      qryCabNota.fieldByName('NUMIMPBOLETO').AsInteger;
    paramByName('TIPO').AsString := 'D';
   end;
end;

procedure TformImprimirRecibo.FormShow(Sender: TObject);
begin
...
procedure TformImprimirRecibo.BitBtn1Click(Sender: TObject);
begin
   close;
end;

procedure TformImprimirRecibo.qryCabNotaAfterOpen(DataSet: TDataSet);
begin
   cbCopias.ItemIndex :=
     pred(Dataset.fieldByName('COPIAS').AsInteger);
end;

procedure TformImprimirRecibo.qryCabNotaBeforeOpen(DataSet: TDataSet);
begin
  with (Dataset as TIBQuery) do
