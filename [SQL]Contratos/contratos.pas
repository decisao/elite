...
type
  TformContratos = class(TForm)
    ...
    qryDetalhesDATAINI: TDateField;
    qryDetalhesDATAFIM: TDateField;
    qryDetalhesCODVENDEDOR: TIntegerField;
    qryDetalhesCODTECNICO: TIntegerField;
    qryDetalhesVALOR: TIBBCDField;
    qryDetalhesCODNATUOPER: TIBStringField;
    qryDetalhesCODSERVICO: TIntegerField;
    qryDetalhesCODCONDPAG: TIntegerField;
    qryDetalhesFATURAR: TIBStringField;
    ...
    qryServico: TIBQuery;
    dsrServico: TDataSource;
    traCondPag: TIBTransaction;
    qryCondPag: TIBQuery;
    dsrCondpag: TDataSource;
    DBCheckBox1: TDBCheckBox;
    Grade1: TMenuItem;
    N2: TMenuItem;
    Notasfiscaisdefatura1: TMenuItem;
    ...
    procedure qryDetalhesCODIGOChange(Sender: TField);
    procedure qryDetalhesCODTECNICOChange(Sender: TField);
    procedure qryDetalhesCODVENDEDORChange(Sender: TField);
    procedure Label12Click(Sender: TObject);
    procedure Label11Click(Sender: TObject);
    procedure qryDetalhesCODNATUOPERChange(Sender: TField);
    procedure qryDetalhesCODSERVICOChange(Sender: TField);
    procedure Label13Click(Sender: TObject);
    procedure qryDetalhesCODCONDPAGChange(Sender: TField);
    procedure Grade1Click(Sender: TObject);
    procedure Notasfiscaisdefatura1Click(Sender: TObject);
  private
...
uses data_principal, clientes, servicos, natuoper, condpag,
  faturar_contratos;
...
procedure TformContratos.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformContratos.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformContratos.btnSairClick(Sender: TObject);
begin
//     close;
end;

procedure TformContratos.FormCreate(Sender: TObject);
begin
...
procedure TformContratos.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformContratos.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformContratos.cmbSelecionarChange(Sender: TObject);
begin
     edtSelecionar.Clear;
     edtSelecionar.SetFocus;
end;

procedure TformContratos.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformContratos.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformContratos.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformContratos.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformContratos.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformContratos.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformContratos.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
...
procedure TformContratos.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformContratos.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformContratos.qryDetalhesAfterOpen(DataSet: TDataSet);
begin
   { mascaras }
   with Dataset do
    begin
   ...
   qryDetalhesCODIGOChange(nil);
   qryDetalhesCODTECNICOChange(nil);
   qryDetalhesCODCONDPAGChange(nil);
   qryDetalhesCODNATUOPERChange(nil);
   qryDetalhesCODSERVICOChange(nil);
   qryDetalhesCODVENDEDORChange(nil);
end;
...
procedure TformContratos.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformContratos.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformContratos.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformContratos.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
end;

procedure TformContratos.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformContratos.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformContratos.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [];
end;

procedure TformContratos.Label5Click(Sender: TObject);
begin
   Screen.Cursor := crHourGlass;
...
procedure TformContratos.Label7Click(Sender: TObject);
begin
   Screen.Cursor := crHourGlass;
   with TformClientes.Create(Self) do
    try
     ...
     ShowModal;
     if (ModalResult = mrOk) and (cdsSelecionar.Active) then
       Self.qryDetalhes.fieldByName('CODVENDEDOR').AsInteger :=
         cdsSelecionar.fieldByName('CODIGO').AsInteger;
    finally
     release;
    end;
...
procedure TformContratos.Label8Click(Sender: TObject);
begin
   Screen.Cursor := crHourGlass;
   with TformClientes.Create(Self) do
    try
     ...
     ShowModal;
     if (ModalResult = mrOk) and (cdsSelecionar.Active) then
       Self.qryDetalhes.fieldByName('CODTECNICO').AsInteger :=
         cdsSelecionar.fieldByName('CODIGO').AsInteger;
    finally
     release;
    end;
...
procedure TformContratos.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
  with Dataset do
   begin
     ...
     fieldByName('FATURAR').AsString := 'S';
   end;
  qryDetalhesCODIGOChange(nil);
  qryDetalhesCODTECNICOChange(nil);
  qryDetalhesCODCONDPAGChange(nil);
...
procedure TformContratos.qryDetalhesCODIGOChange(Sender: TField);
begin
  with qryCliente do
   begin
...
procedure TformContratos.qryDetalhesCODTECNICOChange(Sender: TField);
begin
  with qryTecnico do
   begin
     if active then close;
...
procedure TformContratos.qryDetalhesCODVENDEDORChange(Sender: TField);
begin
  with qryVendedor do
   begin
     if active then close;
...
procedure TformContratos.Label12Click(Sender: TObject);
begin
   Screen.Cursor := crHourGlass;
...
procedure TformContratos.Label11Click(Sender: TObject);
begin
   Screen.Cursor := crHourGlass;
   with TformNatuoper.Create(Self) do
    try
     ...
     ShowModal;
     if (ModalResult = mrOk) and (qrySelecionar.Active) then
       Self.qryDetalhes.fieldByName('CODNATUOPER').AsString :=
...
procedure TformContratos.qryDetalhesCODNATUOPERChange(Sender: TField);
begin
  with qryNatuoper do
   begin
     if active then close;
...
procedure TformContratos.qryDetalhesCODSERVICOChange(Sender: TField);
begin
  with qryServico do
   begin
     if active then close;
...
procedure TformContratos.Label13Click(Sender: TObject);
begin
   Screen.Cursor := crHourGlass;
   with TformCondPag.Create(Self) do
    try
     ...
     if (ModalResult = mrOk) and (qrySelecionar.Active) then
      begin
       Self.qryDetalhes.fieldByName('CODCONDPAG').AsInteger :=
         qrySelecionar.fieldByName('CODIGO').AsInteger;
      end;
    finally
     release;
...
procedure TformContratos.qryDetalhesCODCONDPAGChange(Sender: TField);
begin
  with qryCondpag do
   begin
     if active then close;
...
procedure TformContratos.Grade1Click(Sender: TObject);
begin
...
procedure TformContratos.Notasfiscaisdefatura1Click(Sender: TObject);
begin
  with TformFaturarContratos.Create(Self) do
   try
     showmodal;
   finally
     release;
